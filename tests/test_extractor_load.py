...
class TestUploadPodcastPayloadToS3:
  def test_uploads_json_to_s3(self):
    mock_s3 = MagicMock()
    episodes = [
      {"podcast_id": 1, "title": "Ep1", "audio_link": "https://example.com/ep1.mp3"},
      {"podcast_id": 1, "title": "Ep2", "audio_link": "https://example.com/ep2.mp3"},
    ]
    ...
    assert call_kwargs["Key"] == "podcasts/staging/1/episodes.json"
    assert call_kwargs["ContentType"] == "application/json"
    assert json.loads(call_kwargs["Body"]) == episodes

  def test_uploads_with_correct_s3_key_format(self):
    mock_s3 = MagicMock()
    episodes = [{"title": "Ep"}]

    load.upload_podcast_payload_to_s3(mock_s3, "bucket", 42, episodes)

    call_kwargs = mock_s3.put_object.call_args[1]
    assert call_kwargs["Key"] == "podcasts/staging/42/episodes.json"
...
class TestLoadPodcastEpisodes:
  ...
  def test_returns_uploaded_and_failed_counts(self, mock_build, mock_upload, validated_episode):
    ...
    uploaded, failed = load.load_podcast_episodes(conn, s3, "bucket", podcast_data)
...
class TestLoadAllEpisodes:
  ...
  def test_sums_counts_across_podcasts(self, mock_load_podcast):
    mock_load_podcast.side_effect = [(2, 0), (1, 1)]
    conn, s3 = MagicMock(), MagicMock()
    entries = [
