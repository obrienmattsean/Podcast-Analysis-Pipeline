...
def lambda_handler(event: dict, context: dict = None) -> dict:
    ...
    episode_uri = event.get("episode_uri")
    if not episode_uri:
        return {"statusCode": 400, "body": "Missing episode_uri in event"}
    ...
    try:
        ...
        transcript = fetch_transcript_from_s3(s3_client, episode_uri)
        episode_id = extract_episode_id(episode_uri)
...
if __name__ == "__main__":
    # For local testing
    event = {"episode_uri": "s3://c23-podex-ai-bucket/26/199/"}
    response = lambda_handler(event)
    print(response)
